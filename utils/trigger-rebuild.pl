...
my $AUTH_REALM = 'CKO developer account';

sub request_auth() {
  print(header(-type => 'text/html',
               -status => '401 Authorization Required',
               -WWW_Authenticate => "Basic realm=\"$AUTH_REALM\""),
        start_html('CKO rebuild trigger'),
...
sub do_update($;$) {
  ...
  open STDERR, ">&STDOUT";
  system("sudo", $DGL, "update-$branch", @_);
  print "</pre>";
  if ($?) {
    my $msg;
    if ($? == -1) {
...
sub main() {
  ...
  if (not $ver) {
    do_prompt 'trunk', '0.25', '0.24', '0.23', '0.22', 'hellcrawl', 'bcrawl', 'bcadrencrawl', 'gooncrawl', 'stoatsoup', 'bloatcrawl2', 'kobold-nightstalker';
  } elsif ($ver eq 'trunk') {
    do_update 'trunk';
  } elsif ($ver =~ /^0.25$|^0.24$|^0.23$|^0.22$|^hellcrawl$|^bcrawl$|^bcadrencrawl$|^gooncrawl$|^stoatsoup$|^bloatcrawl2$|^kobold-nighstalker$/) {
