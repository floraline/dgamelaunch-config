...
sub main() {
  ...
  if (not $ver) {
    do_prompt 'trunk', '0.25', '0.24', '0.23', '0.22', 'hellcrawl', 'bcrawl', 'bcadrencrawl', 'gooncrawl', 'stoatsoup', 'bloatcrawl2';
  } elsif ($ver eq 'trunk') {
    do_update 'trunk';
  } elsif ($ver =~ /^0.25$|^0.24$|^0.23$|^0.22$|^hellcrawl$|^bcrawl$|^bcadrencrawl$|^gooncrawl$|^stoatsoup$|^bloatcrawl2$/) {
