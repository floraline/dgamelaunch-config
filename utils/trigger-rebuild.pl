...
sub main() {
  ...
  if (not $ver) {
    do_prompt 'trunk', '0.23', '0.22', 'hellcrawl', 'bcrawl', 'bcadrencrawl', 'gooncrawl', 'stoatsoup', 'two-state-vampire';
  } elsif ($ver eq 'trunk') {
    do_update 'trunk';
  } elsif ($ver =~ /^0.23$|^0.22$|^hellcrawl$|^bcrawl$|^bcadrencrawl$|^gooncrawl$|^stoatsoup$|^two-state-vampire$/) {
